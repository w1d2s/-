 ...
 * 
 * 1. 不同阵营的处理：阵营相同的点之间距离为正无穷INF.
 * 2. 按照y给点排序时，可以将归并排序融入算法，保证复杂度为O(nlogn).
 * 3. merge分界线两边的点的最小距离时，左边每个满足
 ...
 *      以及delta可能为INF的情况.
 * 4. 可能有极端情况出现，最好是开始处理前将坐标系随机旋转一个角度theta，
 *      但这题数据弱，不随机也能过。
 * 
...
void cal_nearest_merge(const int lo_left, const int lo_right,
    ...
    //  注意！ 经过按y归并，这里lo_right处的点已经不是mid_x了！！！
...
int main() {
    ...
    while (t--) {
        ...
        sort(p_arr, p_arr + 2 * n, Cmp_x());

        double min_dist;
        cal_nearest_point_pair(0, 2 * n - 1, min_dist);
